...
def crest(mol, folder=None, path=DEFAULT_RUN_PATH, frozen_idxs=None, crest_path='crest', xtb_path='xtb', spinpol=0, charge=0):
    with log.NoPrint():
        ...
        with open(j(path, folder + '_crest', 'run.sh'), 'w+', newline='') as runsh:
            ...
            if frozen_idxs is not None:
                ...
                constr = ''
            runsh.write(f'{crest_path} coords.xyz -xnam "{xtb_path}" --noreftopo -rthr 1 -c {charge} -u {spinpol} {constr}\n')
