...
def get_xyz(mol):
    s = f'{len(mol.atoms)}\n\n'
    for e, p in zip([atom.symbol for atom in mol.atoms],
                    [atom.coords for atom in mol.atoms]):
...
def load(path):
    ...
    with open(path) as f:
        ...
        for line in flag_lines:
            ...
            key, val = line.split('=')
            val = val.replace("'", '"')
            # split value1_value2_value3 into [value1, value2, value3] and cast
            # to int
            val = [i if not i.isnumeric() else float(i)
                   for i in val.strip().split('_')]
            flags[key.strip()] = val
        ...
        for flag, val in flags.items():
            ...
            if flag.startswith('R'):
                for i in val[:2]:
                    mol.atoms[int(i) -
                              1].identifier_data['labels'].append(flag)
            # simple flags
            ...
            # flags that are ordered
                ...
                for j, i in enumerate(val):
                    mol.atoms[int(i) -
                              1].identifier_data['labels'].append(f'{flag}_{j}')
...
def get_labeled_atoms(molecule, label, origin=None, return_idx=False):
    ...
    if len(atoms) == 0:
        ...
        atoms = [atom[1] for atom in sorted(
            [atom for atom in atoms], key=lambda atom: atom[0])]
    if return_idx:
...
def TSRC(molecule):
    ...
    tsats = {label.split(
        '_')[-1] for atom in atoms for label in atom.identifier_data['labels'] if label.startswith('active_atoms')}
    dists = {}
    for i in tsats:
        atoms_ = get_labeled_atoms(molecule, f'active_atoms_{i}')
        idxs = get_labeled_atoms(
            molecule,
            f'active_atoms_{i}',
            return_idx=True)
        dists[i] = idxs, np.linalg.norm(
            np.array(atoms_[0].coords) - np.array(atoms_[1].coords))

    return dists


if __name__ == '__main__':
    print(load(r"D:\Users\Yuman\Desktop\PhD\ychem\reaction_generation\input_mols\3d9e03fb7103af580fd95a3fa8a820856bcd70bea2d2363709438e1ad5ba10df.xyz"))
